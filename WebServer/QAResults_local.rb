...
class ProdCounts< WEBrick::HTTPServlet::AbstractServlet
  ...
  def print_questions(request)
    ...
    dateName = "#{time.month}/#{time.day}/#{time.year}"
    # html= "<html><META HTTP-EQUIV=\"refresh\" content=\"5;url=http://10.203.65.64:8000/\" ><body><form method='POST' action='/save'>"
    html= "<html><body>"
    html+="<a href=\"/testlisting\"><button>Test Case Listing page</button></a>  "
    html+="<a href=\"/devices\"><button>Devices</button></a>  "
    ...
    results.each(:as => :array) do |row|
      ...
      for i in 0..10
        if i ==0
          ...
          html += "<td style=\"font-size:10px\"><a href=\"http://10.203.65.64:8001/#{row[i]}.png\" target=\"_blank\"><b>  #{row[i]} </b></a><br>"
          if row[11] 
               ...
               artifacts.each do |artifact|
                  ...
                  artifact = artifact.gsub("\"","")
                  artifact.strip!            
                  html += "<a href=\"http://10.203.65.64:8001/#{artifact}\" target=\"_blank\"><b>  #{artifact} </b><br></a>"
               end
          end
...
class Details2 < WEBrick::HTTPServlet::AbstractServlet
  ...
  def get_details(request)
    ...
    if osQry==1
      #query="select test_name,os,browser,enviro,status,date_run,branch,manual_check,id as results_ID,image from results where convert(varchar(10),date_run,10) = convert(varchar(10),DATEADD(day,-#{offset},getdate()),10) and enviro ='#{enviro}' and status ='#{status}' and os ='#{os}' order by test_name"
      query = "select r.test_name,tc.description,r.os,r.browser,r.enviro,r.status,r.date_run,r.branch,r.id as results_ID,r.image, tc.priority from results r left join test_cases tc  on r.test_name =tc.test_name where convert(varchar(10),r.date_run,10) = convert(varchar(10),DATEADD(day,-#{offset},getdate()),10) and r.enviro ='#{enviro}' and r.status ='#{status}' and r.os ='#{os}' order by date_run" 
      if status =="BOTH"
          query = query.gsub!"r.status ='BOTH'","r.status in ('Passed','Failed')" 
      end
      ...
      #query ="select test_name,os,browser,enviro,status,date_run,branch,manual_check,id as results_ID,image from results where convert(varchar(10),date_run,10) = convert(varchar(10),DATEADD(day,-#{offset},getdate()),10) and enviro ='#{enviro}' and status ='#{status}' and browser ='#{browser}' order by test_name"
      query ="select r.test_name,tc.description,r.os,r.browser,r.enviro,r.status,r.date_run,r.branch,r.id as results_ID,r.image, tc.priority from results r left join test_cases tc  on r.test_name =tc.test_name where convert(varchar(10),r.date_run,10) = convert(varchar(10),DATEADD(day,-#{offset},getdate()),10) and r.enviro ='#{enviro}' and r.status ='#{status}' and r.browser ='#{browser}'  order by date_run" 
        if status =="BOTH"
          query = query.gsub!"r.status ='BOTH'","r.status in ('Passed','Failed')" 
      end
      ...
      #query ="select test_name,os,browser,enviro,status,date_run,branch,manual_check,id as results_ID,image from results where convert(varchar(10),date_run,10) = convert(varchar(10),DATEADD(day,-#{offset},getdate()),10) and enviro ='#{enviro}' and status ='#{status}' and branch ='#{branch}' order by test_name"
      query ="select r.test_name,tc.description,r.os,r.browser,r.enviro,r.status,r.date_run,r.branch,r.id as results_ID,r.image, tc.priority from results r left join test_cases tc  on r.test_name =tc.test_name where convert(varchar(10),r.date_run,10) = convert(varchar(10),DATEADD(day,-#{offset},getdate()),10) and r.enviro ='#{enviro}' and r.status ='#{status}' and r.branch ='#{branch}'   order by date_run"
      if status =="BOTH"
          query = query.gsub!"r.status ='BOTH'","r.status in ('Passed','Failed')" 
      end
      # puts "#{query}"
     ...
     # query="select test_name,os,browser,enviro,status,date_run,branch,manual_check,id as results_ID,image from results where convert(varchar(10),date_run,10) = convert(varchar(10),DATEADD(day,-#{offset},getdate()),10) and enviro ='#{enviro}' and status ='#{status}' order by test_name"
      query = "select r.test_name,tc.description,r.os,r.browser,r.enviro,r.status,r.date_run,r.branch,r.id as results_ID,r.image, tc.priority from results r left join test_cases tc  on r.test_name =tc.test_name where convert(varchar(10),r.date_run,10) = convert(varchar(10),DATEADD(day,-#{offset},getdate()),10) and r.enviro ='#{enviro}' and r.status ='#{status}'  order by date_run"
        if status =="BOTH"
          query = query.gsub!"r.status ='BOTH'","r.status in ('Passed','Failed')" 
      end
    ...
    results.each(:as => :array) do |row|
      ...
      for i in 0..10
        if i ==0
          ...
          html += "<td style=\"font-size:10px\"><a href=\"http://10.203.65.64:8001/#{row[i]}.png\" target=\"_blank\"><b>  #{row[i]} </b></a></td>"
        end
      end
      html +="</tr>"
  ...
  def get_trends(request)
    ...
    class SpecificTest < WEBrick::HTTPServlet::AbstractServlet
      ...
      def get_specifics(request)
        ...
        results.each(:as => :array) do |row|
          ...
          html += "<td style=\"font-size:10px\"><a href=\"http://10.203.65.64:8001/#{row[1]}.png \" target=\"_blank\"><b>  #{row[1]} </b></a><br>"
          if row[2] != nil
    ...
    class TestListing < WEBrick::HTTPServlet::AbstractServlet
      ...
      def get_description(request)
        ...
        html   = "<html><body>"
        html+="<a href=\"/testlisting\"><button>Test Case Listing page</button></a>  "
        html +="<a href=\"http://10.203.65.64:8001/doc/top-level-namespace.html\" target=\"_blank\"><button>Methods</button></a>"
        ...
        results.each do |row|
          ...
          if row['testcaselink']
            html += "<a href=\"#{row['testcaselink']}\"><b> link</td>"
          else
            html += "</td>"
...
return(document.cookie.match('(^|; )'+name+'=([^;]*)')||0)[2]
        ...
        client.execute('SET TEXTSIZE 2147483647')
        resultsEnviro =client.execute('select environment_name, description from environments order by tab_order')
        ...
        environments.each do |env|
          ...
          html +="<h2><a href=\"/trends?#{env}\">#{env} </a>  </h2><p>#{description[count-1]}</p>"
          #Passed
          passedVal=0
