...
class SMOKE2_Positive_to_LRM_CDS_with_existing_rpl < Test::Unit::TestCase
  def setup
    @test=Utilities.new
  ...
  def test_smoke2_positive_to_lrm_cds_with_existing_rpl
    @test.login_to_zenith()
    db_id =  @test.get_db_id
    testIdentifier = "Test#{db_id}"
    testFilename = "UKBTP232_1.xlsx"
    secondTestFileName = "UKBTP232_2.xlsx"
...
#set the workflowstatus to LRM load complete through SQl
    ...
    binding.pry
    #Second Rpl with the same name for the next month
    ...
    @test.click_element(:xpath,"//a[contains(text(),'#{testIdentifier}')]","#{testIdentifier} the search dropdown")
    @test.click_element(:xpath,Utilities.siteMap[:reporting_period_logs_add_btn],"Add a Reporting Period Log")
    @test.enter_text(:xpath,Utilities.siteMap[:reporting_period_logs_add_rpl_name_text],"#{testIdentifier}-2ndMonth"," for RPL name")
    ...
    @test.enter_text(:xpath,Utilities.siteMap[:reporting_period_logs_add_end_effective_text],"28022017"," for the End Effective Date")
    @test.click_element(:xpath,Utilities.siteMap[:reporting_period_logs_add_add_btn],"Add button on Add Reporting Period Log")

    @test.put_test_data_files_in_dataconnect_folder(secondTestFileName,destFolder,entityType,fileType,company,testIdentifier,"20170228")
    @test.put_test_data_files_in_dataconnect_folder(autoSchemaFile,"5.AutoSchema",entityType,fileType,company,testIdentifier,"20170201")
    workflowStatus = @test.get_element_text(:xpath,"//span[contains(text(),'#{testIdentifier}-2ndMonth')]/../../td[4]/span","Workflow Status of #{testIdentifier}-2ndMonth")
    newTestIdentifier = "#{testIdentifier}-2ndMonth"
    rplId = @test.get_rplId(newTestIdentifier)
    
    result = @test.wait_for_expected_workflow_status(newTestIdentifier,"Ready for Review",10)
    assert(result)
    ...
    @test.right_click_element(:xpath,"//span[contains(text(),'#{testIdentifier}-2ndMonth')]","Right clicking on a row with the expected RPL name of #{testIdentifier}-2ndMonth")
    @test.click_element(:xpath,"//span[contains(text(),'Edit')]","Edit in right click menu for #{testIdentifier} row")
    ...
    @util.logging("Got the message \n #{confirmationText}")
    @test.click_element(:xpath,"//span[contains(text(),'Yes')]", "Yes button on confirmation window")
    result = @test.wait_for_expected_workflow_status(newTestIdentifier,"Completed",10)
    assert(result)
    ...
    @test.click_element(:xpath,Utilities.siteMap[:reporting_period_logs_load_to_lrm_cds_modal_load_btn],"Load button")
    result = @test.wait_for_expected_workflow_status(newTestIdentifier,"LRM CDS Load Success",10)
    assert(result)
