...
class FindLinksOnPage < Test::Unit::TestCase
 ...
 # @test.goto_url("https://staging.zipixy.com/")
   ...
   # @test.enter_text(:xpath,"//input[@id='Email']","chrisgoodnight@gmail.com", "Sign in Email")
...
def safe_click(id)
   begin
	 element = @driver.find_element(:id,"#{id}")
      ...
      @util.logging "the element is not interactable"
      true
      return element
  end
end
def safe_find(text)
	begin
	element = @driver.find_element(:xpath,"//a[contains(text(),'#{text}')]/..")
       return element
     rescue Selenium::WebDriver::Error::NoSuchElementError
      @util.logging "Could not find #{text}"
      false
  end
end